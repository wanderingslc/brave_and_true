...
(defn pegged?
  "Does the board have a peg in it?"
  [board pos]
  (get-in board [pos :pegged]))

(defn remove-peg
  "remove the peg at the given position out of the board"
  [board pos]
...
(defn move-peg
  "take peg out of p1 and move it to p2"
  [board p1 p2]
  (place-peg (remove-peg board p1) p2))

(defn valid-moves
  "return a map of all the valid moves for pos, where key is the destination and value is jumped position"
  [board pos]
  (into {}
...
(defn make-move
  "move pag from p1 to p2, removing the jumped peg"
  [board p1 p2]
...
(defn letter->pos
  "Converts letter string to matching number position"
  [letter]
...
(defn user-entered-valid-move
  "Handles next step after good move passed in"
  [board]
  (if (can-move? board)
    (prompt-move board)
    (game-over board)))
